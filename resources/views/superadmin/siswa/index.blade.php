...
<style>
    ...
    .page-header-badge-dot {
        ...
        background: #818cf8; box-shadow: 0 0 8px rgba(129,140,248,0.8);
    }
...
<main class="flex-1 flex flex-col max-h-screen" style="background: #080e1a;">
    ...
    <div class="flex-1 overflow-y-auto p-8"
         style="background: radial-gradient(ellipse at 80% 0%, rgba(99,102,241,0.05) 0%, transparent 55%), #080e1a;">
        <div class="max-w-7xl mx-auto space-y-7">
            ...
            <div>
                ...
                <h2 class="text-2xl font-bold text-white" style="font-family: 'Orbitron', monospace; text-shadow: 0 0 20px rgba(99,102,241,0.25);">
                    Manajemen Akun Siswa
                </h2>
                <p class="text-slate-500 text-sm mt-1">Manajemen dan monitoring data Siswa aktif dalam sistem.</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="stat-card stat-card-indigo">
                    ...
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">Total Siswa</p>
                    <p class="text-4xl font-bold text-white" style="font-family: 'Orbitron', monospace;">{{ $totalSiswa }}</p>
                </div>
                ...
                <div class="stat-card stat-card-purple">
                    ...
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">Siswa Aktif</p>
                    <p class="text-4xl font-bold text-white" style="font-family: 'Orbitron', monospace;">{{ $siswaAktif }}</p>
                </div>
                ...
                <div class="stat-card stat-card-red">
                    ...
                    <p class="text-4xl font-bold text-white" style="font-family: 'Orbitron', monospace;">{{ $siswaTidakAktif }}</p>
                </div>
                ...
                <div class="stat-card stat-card-blue">
                    ...
                    <p class="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-1">Akun Dibuat</p>
                    <p class="text-4xl font-bold text-white" style="font-family: 'Orbitron', monospace;">{{ $siswaBaru }}</p>
                </div>
            </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <!-- Search & Filters -->
                <form method="GET" action="{{ route('superadmin.siswa') }}" class="flex flex-wrap items-center gap-3 w-full md:w-auto">
                    <!-- Search -->
                ...
                <div class="flex flex-col sm:flex-row gap-3 shrink-0">
                    <a href="{{ route('superadmin.siswa.template') }}" class="btn-indigo-outline">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                        </svg>
                        Download Template
                    </a>
                    <a href="{{ route('superadmin.siswa.create') }}" class="btn-green-solid">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form action="{{ route('superadmin.siswa.destroy', $data->id) }}"
                                          method="POST" id="deleteForm-{{ $data->id }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus siswa ini secara permanen?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-base font-semibold text-white mb-1">Belum Ada Data Siswa</h3>
                                    <p class="text-sm text-slate-500 max-w-sm">
                                        Saat ini belum ada data siswa yang terdaftar. Anda bisa menambahkannya secara manual atau mengimpor data.
                                    </p>
                                </div>
                            </td>
                        </tr>
    ...
    <div style="background: #080e1a; border-top: 1px solid rgba(99,102,241,0.08);">
        <!-- Terminal log -->
        <div class="px-6 py-2 border-b font-mono text-[11px] text-slate-600 tracking-wider overflow-hidden whitespace-nowrap"
             style="border-color: rgba(99,102,241,0.06);">
            <div class="w-full overflow-hidden">
                <div class="animate-ticker gap-8">
                    @forelse($logs as $log)
                        <span>
                            &gt; [{{ $log['waktu'] }}]
                            <span class="text-{{ $log['tipe_aksi'] == 'DELETE' ? 'red' : ($log['tipe_aksi'] == 'UPDATE' ? 'amber' : 'indigo') }}-400">
                                {{ $log['tipe_aksi'] }}
                            </span>: {{ $log['deskripsi'] }}&nbsp;&nbsp;&nbsp;
                        </span>
                    @empty
...
<script>
    function confirmDelete(id, studentName) {
        Swal.fire({
            ...
            html: "Data siswa <span class='font-bold text-indigo-400'>" + studentName + "</span> akan dihapus secara permanen!",
            icon: 'warning',
